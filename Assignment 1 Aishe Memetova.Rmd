...
```{r namesFre_NA, eval=TRUE}
names(ToPlot)
```

We could alter the fifth label:
```{r alterTable, eval=TRUE}
...
```{r pieTitle_suggestion, eval=TRUE}
# the pie plots the table:
...
```{r 100convert, eval=TRUE}
ToPlot*100
```

* You can customize the colors:

```{r rainbowColor, eval=TRUE}
# details:
...
```{r percentExample, eval=TRUE}
paste0(round(ToPlot,2),'%')
```

Then,

```{r labelPercent, eval=TRUE}
...
paletteHereNew=c('gray') # just one color #c for color 
# plotting
barplot(ToPlot,
     col = paletteHereNew,
     border=NA, #no border
     main=titleText,
     sub=sourceText,
     ylim=c(0,50), # y axis measures - fmor 0 to 50
     ylab = '(in %)' # show unit
     )
```
...
tableFreq=as.data.frame(ToPlot) # table converted to data frame 
...
```{r ggplot_base, eval=TRUE}
(x = locale, #the columns which ggplot will use to plot 
                 y = pct)) 
...
```{r infoHIST_base, eval=TRUE}
dataHist
```

And now see the info that was used in ggplot:
```{r infoHIST_gg, eval=TRUE}
...
```{r baseLikeGG, eval=TRUE,fig.height=7}
# base
...
```{r tableIntervals, warning=FALSE,results='asis', eval=TRUE}
...
```{r bixplotMeasurement,eval=TRUE}
